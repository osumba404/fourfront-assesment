...
@section('content')
    ...
    <div class="grid gap-6 sm:grid-cols-2">
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-4 flex gap-3">
                ...
                <form action="{{ route('users.show.form') }}" method="get" class="flex gap-2">
                    <input type="number" name="id" min="1" placeholder="User ID" class="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-4 flex gap-3">
                <a href="{{ route('wallets.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">
                    Create wallet
                </a>
                <form action="{{ route('wallets.show.form') }}" method="get" class="flex gap-2">
                    <input type="number" name="id" min="1" placeholder="Wallet ID" class="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                    <button type="submit" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                    View wallet
                    </button>
                </form>
