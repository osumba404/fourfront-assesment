        ...
        <form action="{{ route('wallets.store') }}" method="post" class="mt-6 max-w-md space-y-4">
            ...
            <div>
                ...
                <select name="user_id" id="user_id" required
                    class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 @error('user_id') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="currency_code" id="currency_code" value="{{ old('currency_code', 'USD') }}" maxlength="3" placeholder="USD"
                    class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 @error('currency_code') border-red-500 @enderror">
                @error('currency_code')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="initial_balance" id="initial_balance" value="{{ old('initial_balance', 0) }}" min="0" step="0.01"
                    class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 @error('initial_balance') border-red-500 @enderror">
