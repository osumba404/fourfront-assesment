...
@section('content')
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        ...
        <div class="mt-8 border-t border-slate-200 pt-6">
            ...
            @if ($wallets->isEmpty())
                <p class="mt-2 text-slate-600">No wallets yet. <a href="{{ route('wallets.create') }}" class="font-medium text-green-600 hover:underline">Create one</a>.</p>
            @else
                <ul class="mt-4 space-y-3">
                    @foreach ($wallets as $wallet)
                        <li class="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50/50 px-4 py-3">
                            <div>
                                <a href="{{ route('wallets.show', $wallet) }}" class="font-medium text-slate-800 hover:text-green-600">{{ $wallet->name }}</a>
                            ...
                            <a href="{{ route('wallets.show', $wallet) }}" class="ml-2 rounded bg-green-600 px-3 py-1 text-sm font-medium text-white hover:bg-green-700">View</a>
                        </li>
                    @endforeach
                </ul>
            @endif
